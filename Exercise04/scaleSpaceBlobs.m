...
for i=1:Nscales
    ...
    % order derivative filters, gxx and gyy, defined above, and store the 
    % result into temporary variables tmpxx and tmpyy, as suggested below, 
    % then you can utilize the separability of gxx and gyy to gain speed.
    % 
    % You can find the 1D filters through the singular value decomposition
    % of gxx and gyy as described in Szeliski's book (Section 3.2.1). 
    % See also Matlab built-in function "svd".
    %
    % Another alternative is to directly filter the image "img" 
    % with the non-separable filter in "scaleNormalizedLaplacian{i}". 
    % This gives the same result but is slightly slower. See "imfilter".
